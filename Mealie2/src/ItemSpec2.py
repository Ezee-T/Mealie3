...
def spec_handler_xls_ora(er):
    ...
    q_write_interface = """
    INSERT INTO maximo.mxitemspec_iface (itemnum, description, Rotating, lottype, capitalized, 
                    ...
                    numvalue, measureunitid, alnvalue, changedate, is_changeby, orgid, itemspecid, transid, transseq)
    SELECT itemnum, :itemdesc, Rotating, lottype, capitalized, 
                    outside, sparepartautoadd, 
                    :classid, inspectionrequired, sendersysid, attachonissue, commodity, 
                    :commid, conditionenabled, iskit, issueunit, 
                    itemid, itemsetid, itemtype, metername, orderunit, prorate, :assetattrid, 
    ...
    for this_row in range(start_row, wsh.nrows):
            ...
            if (ic.is_valid(curs_target_lookup, er.logger)):
                    ...
                    if (data_type == 'ALN'):
                        ...
                        if len(str(valuealn)) > 255:
                            er.logger.error("ERROR: Value for Attribute %s is too long. Value will be amended to fit database column" % (ic.propname))
                            valuealn = valuealn[:-(len(valuealn) - 255)]
                            er.logger.debug("New Attribute Value: %s" % (valuealn))
                    ...
                    curs_target_mea.execute(q_write_interface, itemnum=ic.itemnum,
                                                                    ...
                                                                    transid=trans_id, 
                                                                    transseq=trans_seq)
                    er.logger.debug("Item Description: %s" % (ic.longdesc))
