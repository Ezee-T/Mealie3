...
def format_contract_dt(dt_raw, dt_type, ofh, ifh):
   ...
   if (dt_type == xlrd.XL_CELL_EMPTY):
      retval = 'NULL'      # Contract start and end date are optional so this is allowed
...
"""
   ...
   for this_row in range(start_row, wsh.nrows):
      ...
      pmnum = p.sub('&\'||\'', wsh.cell(this_row,1).value)
      
      
      lstartdate = format_contract_dt(wsh.cell(this_row,3).value, wsh.cell(this_row,3).ctype,  ofh, ifh)
      #= p.sub('&\'||\'', str(int(wsh.cell(this_row,3).value)))
      lcompdate = format_contract_dt(wsh.cell(this_row,4).value, wsh.cell(this_row,4).ctype,  ofh, ifh)
      #= p.sub('&\'||\'', str(int(wsh.cell(this_row,4).value)))
      nextdate = format_contract_dt(wsh.cell(this_row,5).value, wsh.cell(this_row,5).ctype,  ofh, ifh)
      #= p.sub('&\'||\'', str(int(wsh.cell(this_row,5).value)))
      
      ofh.write("UPDATE maximo.pm")
      ofh.write(" SET pm.laststartdate = TO_DATE('%s', 'YYYMMDD'), pm.lastcompdate = TO_DATE('%s', 'YYYYMMDD'), pm.nextdate = TO_DATE('%s', 'YYYMMDD')" %(lstartdate, lcompdate, nextdate))
      ofh.write(" WHERE pm.pmnum = '%s'" %(pmnum))
      ofh.write("\n/\n")
